...
from nets.focal_loss import FocalLoss
from nets.ghm_loss import GHMC_Loss
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, input, targets=None):
        ...
        FOCAL = FocalLoss(gamma=2, alpha=0.1, pred=conf, target=mask,reduction="none")
        
        

        loss_conf = torch.sum(FOCAL(conf,mask)* mask) +  torch.sum(FOCAL(conf,mask)* noobj_mask)
