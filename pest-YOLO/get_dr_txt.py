...
class mAP_Yolo(YOLO):
    ...
    def detect_image(self,image_id,image):
        ...
        with torch.no_grad():
            ...
            #                                           nms_thres=self.iou)
            batch_detections = confluence_process(output,conf_thres=self.confidence)

            #--------------------------/home/wcj/chr/yolov4-pytorch-new/logs/Epoch95-Total_Loss7.2078-Val_Loss7.9952.pth-------------------------------#
            #   如果没有检测出物体，返回原图
            #---------------------------------------------------------#
            try:
            ...
            #   对预测框进行得分筛选
            #---------------------------------------------------------#
            top_index = batch_detections[:,4] * (batch_detections[:,5]+1) > self.confidence
            top_conf = batch_detections[top_index,4]*(batch_detections[top_index,5]+1)
